...
- HTTPS to NVD is verified against the operating-system certificate store (through the `truststore` package), so an HTTPS-inspecting proxy or antivirus whose CA is installed in Windows works. If the report still shows a TLS verification error, set `REQUESTS_CA_BUNDLE` to your organization's CA file.
- The same lookup also reports the release's **end-of-support status** from [endoflife.date](https://endoflife.date), a free and community-maintained dataset, for FortiOS, PAN-OS, Cisco IOS XE and F5 BIG-IP. Only the product name is sent there, never the version. A release train (IOS-XE `17.9`) is enough for this check.
...
- Qualified families: IOS, IOS-XE, ASA, FortiOS, Junos, ScreenOS, PAN-OS, Arista EOS, SonicOS, ArubaOS-Switch and F5 BIG-IP. ArubaOS-Switch 16.x is looked up as `hpe:arubaos-switch` (without the `YA.`/`WC.` prefix). For BIG-IP every module provisioned in `sys provision` is queried as its own NVD product. PIX, pre-16 ProCurve and Check Point policy exports are reported as not supported.
