...
Secret visibility is deliberately not an assessment-policy field. Only the explicit CLI `--show-secrets` option can add parser-qualified unmasked credential source lines to a report; default findings and inventory remain redacted. See the README and security policy before using it.
...
`device_lifecycle` accepts `unknown` (default), `provisioning`, or `commissioned`. Only an explicit `commissioned` declaration enables the bounded Cisco IOS/XE SC-006 findings for an exported `boot host/network tftp:` configuration fetch on qualified releases and for a CNS configuration agent (`cns config initial|partial`) without `encrypt`. This declaration is an auditor-supplied lifecycle fact, not inferred from the configuration; it does not assess PnP, CNS server authentication, controller trust, or whether a server was contacted at runtime. URLs, paths and CNS endpoints are redacted from report evidence.
...
An assessed external Junos interface also scopes narrow SRX screen checks: the selected model must identify SRX and the zone must actively bind the interface and screen. Explicitly deactivated TCP SYN-flood, UDP-flood or ICMP-flood options are reported, as are active UDP/ICMP-flood options paired with explicit `alarm-without-drop`. Missing options, group inheritance, unbound screens and EX models are not graded as disabled protection.

Explicit interface roles currently enable trust-boundary CDP/LLDP analysis on IOS/XE and Junos and access-edge analysis on IOS/XE and AOS-S, plus explicit BPDU-guard override/filter checks on EOS access-edge ports and explicit 802.1X force-authorized and BPDU-protection disable/filter checks on AOS-S access-edge ports. The IOS/XE access-edge role additionally scopes explicit 802.1X forced-authorization, open-access and global-disable bypass checks, plus effective BPDU-guard override/filter checks; it does not make omitted 802.1X or BPDU-guard configuration a finding. A PAN-OS interface explicitly assigned `external` also scopes narrow attached-zone-profile SYN/UDP/ICMP-flood-disablement and scan-allow checks; zone names are never treated as roles. Possible DoS alternatives suppress the flood checks, not scan-allow; unresolved Panorama state suppresses both. Exact protected-AAA declarations qualify path evidence for FortiOS and Junos administrative RADIUS records. Device lifecycle gates the narrow IOS/XE TFTP boot-config check, and exact approved DH groups gate ASA's additional IKE profile finding. Configuration-backup scope controls only the absence decision for qualified device-side schedules. Credential length and blocklist policy applies to secret-free IOS/XE, ASA, Junos, ScreenOS, and EOS metadata. Explicit certificate inputs enable bounded PAN-OS, ASA, FortiOS and selected-trustpoint IOS/XE public-material assessment. Other plugins may use context only after their own vendor grammar, authoritative security requirement, and adversarial tests are added.

CIDR targeting, configuration-section selection, threshold overrides, and overlapping selector precedence are intentionally unsupported. Report inventory selection does not change parsing or finding evaluation. Scanning only part of a configuration is out of scope (maintainer decision, 2026-09-25): selecting part of an export could remove referenced objects, alter rule ordering, or suggest that excluded data is secure.
