...
| <a id="S-JUN"></a>S-JUN | Hardening Junos Devices Checklist, companion to This Week: Hardening Junos Devices, Second Edition; current Junos login, accounting, J-Web session and NTP references | Checklist copyright 2015, two pages; second edition remains listed in vendor book catalogue. Current login-class, login-message, RADIUS/TACACS+ accounting and J-Web session semantics were verified 2026-09-14. Current NTP key/type/trust guidance was verified 2026-09-13 and preserves documented MD5-only platform exceptions; do not adopt the checklist's obsolete SHA1 storage advice | Chapter 4 checklist: management, access, authentication, routing and firewall filters; named login-class and global idle-timeout precedence; pre-login message versus post-login announcement; accounting event/destination resolution; explicit J-Web session settings; Chapter 2 physical ports; per-association NTP key authentication and trusted-key resolution | [Hardening checklist](https://www.juniper.net/assets/us/en/local/pdf/books/tw-hardening-junos-devices-checklist.pdf); [login classes](https://www.juniper.net/documentation/us/en/software/junos/cli-reference/topics/ref/statement/class-edit-system-login.html); [login settings](https://www.juniper.net/documentation/us/en/software/junos/user-access/topics/topic-map/junos-os-login-settings.html); [TACACS+ accounting](https://www.juniper.net/documentation/us/en/software/junos/user-access/topics/topic-map/user-access-tacacs-authentication.html); [RADIUS accounting](https://www.juniper.net/documentation/us/en/software/junos/user-access/topics/topic-map/user-access-radius-authentication.html); [J-Web sessions](https://www.juniper.net/documentation/us/en/software/junos/cli-reference/topics/ref/statement/session-edit-system.html); [NTP authentication](https://www.juniper.net/documentation/us/en/software/junos/time-mgmt/topics/concept/ntp-authentication-keys.html) |
...
| <a id="S-PAN-M"></a>S-PAN-M | PAN-OS Help: Device > Setup > Management; Device > Administrators/Admin Roles; Configure an SSH Service Profile; PAN-OS 11.2 CLI hierarchy | 12.1 management/admin-role bodies and current SSH guidance verified 2026-09-14. Explicit management fields and PAN-OS 10+ attached SSH profiles are qualified; FIPS-dependent/absent defaults, older grammar, upstream MFA and unmerged Panorama state are not inferred | Minimum Password Complexity; username inclusion and password reuse; Authentication and General Settings; administrator role/profile bindings; banners; idle/lockout/session settings; TLS and management SSH service profiles | [Management](https://docs.paloaltonetworks.com/ngfw/help/12-1/device/device-setup-management); [Administrators](https://docs.paloaltonetworks.com/ngfw/help/10-2/device/device-administrators); [Admin roles](https://docs.paloaltonetworks.com/ngfw/help/12-1/device/device-admin-roles); [SSH profile](https://docs.paloaltonetworks.com/ngfw/administration/certificate-management/configure-ssh-service-profile); [CLI hierarchy](https://docs.paloaltonetworks.com/ngfw/pan-os-cli-quick-start/cli-command-hierarchy/pan-os-11-2-configure-cli-command-hierarchy) |
...
| <a id="S-HP"></a>S-HP | Aruba 2530/2930M/F/3810M/5400R Access Security Guides and AOS-S 16.10/16.11 Basic Operation Guides | 16.10/16.11 running-config syntax and defaults verified through 2026-09-14. Administrative inference is limited to those release families; absent credentials require `include-credentials` or explicit removal, and applicability excludes AOS-CX | Manager/operator password protection; per-channel login/enable authentication and `authorized` behavior; remote CLI and serial/USB idle timeout; WebAgent idle timeout and independent plaintext/TLS enablement; MOTD; authorized managers; SNMPv3 per-user protection | [Password security](https://arubanetworking.hpe.com/techdocs/AOS-S/16.11/ASG/YC/content/common%20files/cnf-pas-sec.htm); [authentication methods](https://arubanetworking.hpe.com/techdocs/AOS-S/16.10/ASG/YC/content/common%20files/cnf-swi-aut-met.htm); [16.11 operations](https://arubanetworking.hpe.com/techdocs/AOS-Switch/16.11/Aruba%20Basic%20Operation%20Guide%20for%20AOS-S%2016.11.pdf); [SNMPv3 users](https://arubanetworking.hpe.com/techdocs/AOS-S/16.11/MCG/WC/content/common%20files/snm-use-com.htm) |
...
| Junos | P | P | P | P | P | P | P | P | A | P | P | A | A | P |
| ScreenOS | P | P | P | P | P | P | P | P | A | P | P | A | U | A |
| Check Point FW1 policy export | U | U | U | U | P | U | U | U | U | P | U | U | U | U |
| PAN-OS local XML | P | P | P | P | P | P | P | P | A | P | P | P | A | A |
| HP_PROCURVE / AOS-S | P | P | P | P | P | P | P | P | A | A | P | A | A | A |
...
**Current coverage:** Root SSH, remote auth order, retry/lockout, superuser credentials, explicit SSH algorithms and legacy services remain in place. GAP-017 adds typed named-user login-class resolution with class/global idle-timeout precedence, separate pre-login message and post-login announcement state, required login/change-log/interactive-command accounting events and resolved RADIUS/TACACS+ destinations when remote authentication is selected, and explicit J-Web idle/session-limit checks. Unexpanded `apply-groups`, malformed values and the disputed absent J-Web idle-timeout default remain unknown.

**What's missing:** Complete class-permission adequacy and least-privilege policy cannot be inferred without organizational role intent; MFA applicability, emergency-account exceptions, external accounting delivery and active browser sessions require policy or operational evidence. Broader password policy and current cryptographic recommendations remain separate work. Controller/group inheritance expansion is not implemented.
...
**Current coverage:** The existing all-local administrator warning, password policy and management access/TLS checks remain. GAP-017 adds typed dynamic/custom administrator roles, authentication-profile resolution and explicit MFA state; login banner/acknowledgement; explicit idle, failed-attempt, lockout and concurrent-session controls; and attachment-driven PAN-OS 10+ management SSH profile resolution. New findings are limited to locally proven missing/unresolved references, explicit unsafe values, missing banner state, and missing/unresolved/empty or explicitly legacy applied SSH profiles.

**What's missing:** Organizational role suitability and emergency-account policy, upstream RADIUS/SAML MFA enforcement, authentication server transport/availability, FIPS-dependent absent defaults, pre-10 SSH grammar, active session state, and whether an SSH service restart activated a committed profile remain unassessed. Panorama inheritance remains explicit unknown unless a merged effective export is supplied.
...
**Current coverage:** 16.10-specific service defaults, explicit SSH algorithm checks, AAA/accounting, password-control flag, remote logging, SNTP servers and VLAN DHCP snooping exist. AOS-S 16.10/16.11 now resolves tagged/untagged membership, DHCP/ARP trust, ARP-protected VLANs, Dynamic IP Lockdown and port security for explicitly classified access edges without duplicating the VLAN DHCP finding. GAP-017 additionally resolves ordered manager/operator credentials, per-channel login/enable methods and applicability, remote CLI/serial/WebAgent idle timeouts, MOTD, and independent WebAgent plaintext/TLS service state. Findings require a proven local-credential absence, documented unauthenticated method, known missing notice, or applicable known timeout outside the project target.

**What's missing:** Certificate material and served-chain validation, organizational role suitability, emergency-account policy, runtime authentication/session state, broader product-family/version tables, DHCPv6/ND protections, binding-database contents and non-2530/2930/3810/5400R syntax need dedicated evidence and fixtures. Do not substitute AOS-CX manuals.
...
**Current coverage:** SonicOS 7 custom E-CLI now exposes typed built-in/local administrators, recursive administrative-group membership and precedence, TOTP and secret-free credential presence, authentication method/local fallback, password minimum/complexity/scope, lockout/session/CLI-attempt controls, CLI banners, and active HTTPS TLS/certificate selection. Explicit commands are evaluated across supported 7.x exports. Omitted documented values are reconstructed only for identified 7.0-7.2 releases; 7.3+ omissions remain unknown because new-install and upgrade behavior differs. Normalized users intentionally remain unknown because the custom export cannot prove the built-in credential value or complete external identity state.

**What's missing:** Static custom exports cannot prove the built-in password value, upstream MFA execution, runtime authentication outcome, certificate actually served, trust-store acceptance, revocation, or management-path reachability. Legacy preference exports remain a separate unsupported dialect. Full exported X.509 material resolution remains under GAP-013 rather than being inferred from a certificate-selection name.
...
**Current coverage:** GAP-017 adds typed local administrator roles/default-role resolution, ordered login/EXEC/all-command AAA and accounting, lockout, applicable console/SSH/Telnet timers, distinct login/MOTD banners, and active eAPI SSL-profile/certificate/explicit-TLS resolution. The centralized-authentication check now applies to explicitly active CLI management as well as eAPI. Existing credential-storage, SSH algorithms/empty-password/ACL, SNMP, logging and authenticated-time checks remain.

**What's missing:** Organizational role suitability, emergency-account procedure, upstream AAA/MFA execution, runtime sessions, ACL content semantics, implicit SSL-profile TLS defaults, certificate material/chain/trust/revocation and the certificate actually served remain unassessed. Parsed hash-only users do not receive guessed plaintext-strength verdicts. Control plane/routing/discovery remain STD-002/003/008.

**Why it matters:** CLI-only administration and each active eAPI service attachment now receive native EOS assessment rather than relying on inherited IOS syntax.
...
| Junos | [junos.py](../../src/devices/juniper/junos.py) | Hierarchical/set/delete/deactivate/activate effective statements; unexpanded apply-groups identified. Typed users, login-class/idle-timeout bindings, pre/post-login notices, accounting events/destinations, J-Web session controls, stateless filters, SRX zone-pair policies, address/application resolution, attached VPN reference chains, logging and crypto. Global/dynamic policies, expanded groups and runtime installation/session/SA state are not modeled. |
...
| PAN-OS | [panos.py](../../src/devices/paloalto/panos.py) | Local device/vsys XML, management attachments, typed administrator role/authentication/MFA evidence, explicit banner/session/lockout settings, attached PAN-OS 10+ management SSH profiles, password settings, schedule families and forwarding. Panorama inheritance is flagged and never merged. TLS profiles and exported public certificate objects are resolved at the documented static boundary; runtime service state remains unknown. |
| AOS-S | [procurve.py](../../src/devices/hp/procurve.py) | Ordered service flags, manager/operator credential completeness/removal, per-channel login/enable methods, remote/serial/WebAgent idle timeouts, MOTD, v3 algorithm users, AAA, password flag, logging/SNTP, VLAN/snooping. General service defaults are limited to 16.10 and administrative defaults to verified 16.10/16.11 exports; not AOS-CX. |
...
| [hp/hp_checks_plugin.py](../../src/analyze/hp/plugins/hp_checks_plugin.py) — `check_administrative_policy` | `hp.procurve.admin.manager_credential_missing`; `hp.procurve.admin.unauthenticated_method`; `hp.procurve.admin.login_banner`; `hp.procurve.admin.remote_cli_idle_timeout`; `hp.procurve.admin.serial_idle_timeout`; `hp.procurve.admin.web_idle_timeout` | Applicable per-channel authentication, proven local manager protection, MOTD and independent remote CLI/serial/WebAgent idle-timeout policy. Hidden credentials, malformed values, unknown releases, certificates and runtime state remain ungraded. |
...
| [junos/baseline_plugin.py](../../src/analyze/juniper/junos/plugins/baseline_plugin.py) — `check_authentication` | `juniper.junos.authentication.centralized`; `juniper.junos.authentication.lockout`; `juniper.junos.authentication.login_attempts`; `juniper.junos.authentication.super_user`; `juniper.junos.authentication.weak_storage` | Known version/parser scope: radius/tacplus order, retries>3, missing lockout, superuser missing auth, plaintext/$1$/$9$/$md5$ storage. Broader password policy remains out of scope. |
| [junos/baseline_plugin.py](../../src/analyze/juniper/junos/plugins/baseline_plugin.py) — `check_administrative_policy` | `juniper.junos.authentication.login_banner`; `juniper.junos.authentication.login_class_binding`; `juniper.junos.authentication.idle_timeout`; `juniper.junos.authentication.accounting_events`; `juniper.junos.authentication.accounting_destination`; `juniper.junos.administration.web_session_timeout`; `juniper.junos.administration.web_session_limit` | Pre-authentication notice, named-user class resolution, effective CLI timeout, remote-auth accounting coverage/destination and explicit J-Web session bounds. Apply-groups, malformed values, policy-specific privilege adequacy, MFA and runtime delivery/session state remain unknown or deferred. |
...
| [panos/panos_checks_plugin.py](../../src/analyze/paloalto/plugins/panos_checks_plugin.py) — `check_administration` | `paloalto.panos.admin.centralized_authentication` | Warning when administrators all use local authentication; no remote transport or upstream MFA enforcement claim. |
| [panos/panos_checks_plugin.py](../../src/analyze/paloalto/plugins/panos_checks_plugin.py) — `check_administrative_policy` | `paloalto.panos.admin.role_assignment`; `paloalto.panos.admin.authentication_profile_unresolved`; `paloalto.panos.admin.login_banner`; `paloalto.panos.admin.login_banner_acknowledgement`; `paloalto.panos.admin.idle_timeout`; `paloalto.panos.admin.login_attempts`; `paloalto.panos.admin.lockout_time`; `paloalto.panos.admin.concurrent_sessions`; `paloalto.panos.admin.ssh_profile_missing`; `paloalto.panos.admin.ssh_profile_unresolved`; `paloalto.panos.admin.ssh_profile_algorithms` | Local role/authentication-profile resolution, explicit banner/session/lockout fields and applied PAN-OS 10+ management SSH algorithms. Panorama, malformed/absent numeric state, pre-10 grammar, upstream MFA and runtime SSH activation remain unknown. |
