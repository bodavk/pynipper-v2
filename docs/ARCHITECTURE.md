...
`src/report/report.py` renders findings and optional software advisories as HTML or JSON. The neutral `Finding.to_dict()` contract remains the issue JSON boundary. Plugins pass parser `ConfigEvidence` objects as finding evidence where available; `Finding` keeps the text in `evidence` and adds `evidence_locations` with text, line, file name and `line_origin`. Before filtering, every processor calls `attach_source_lines`, which fills a missing line only when the evidence text equals exactly one input line (`source-match`); repeated, redacted, summarized and absence statements keep no line. Reports never guess a location.
