...
Junos screen analysis follows that boundary: the parser resolves active SRX zone-to-screen and zone-to-interface bindings before exposing explicit TCP SYN-flood, UDP-flood, ICMP-flood and alarm-without-drop state. The plugin checks only a screen bound to an auditor-declared external interface on a model identified as SRX. Unexpanded groups suppress findings, and a missing screen option is unknown rather than an inferred disablement; IDP policy resolution is a separate future stage.
...
The CLI-only `--show-secrets` path is a narrow report exception to the secret-free evidence contract. Normal parsers, normalized records, plugins, findings and inventory stay sanitized. For supported families, the report adapter uses effective parser-owned credential metadata and its source line numbers; FortiOS uses effective secret fields, F5 TMOS uses explicit `auth user` password directives, and SonicOS 7 uses explicit built-in/local administrator password directives, each with a verified input snapshot. The adapter adds a separate sensitive appendix. Cisco IOS/IOS-XE (and EOS through the shared parser) and ASA additionally expose `get_report_secret_evidence()`, which collects secret-bearing lines from typed records that already resolved ordering, removal and binding: SNMP communities/v3 keys, referenced NTP keys, and IOS BGP passwords and bound OSPF/RIP/EIGRP keys. They also scan a small set of key directives with ordered removal and parent-block scope: IOS TACACS+ keys, ISAKMP keys and keyring pre-shared keys; ASA tunnel-group pre-shared keys and AAA server keys. Junos returns active (non-deactivated, non-`delete`d) statements whose leaf is a secret token (`secret`, `authentication-key`, `pre-shared-key`, v3 passwords and similar), SNMP community names and `md5 ... key` statements, for both set and hierarchical formats. AOS-S adds effective SNMP communities, SNMPv3 users with key material, SNTP keys, and RADIUS/TACACS+ global and per-host keys with `no` removal. ScreenOS uses its effective (`unset`-aware) command list for SNMP communities, auth-server secrets, IKE gateway pre-shared keys and NTP server keys. PAN-OS returns only the matching secret XML element with its start line, because exports often place many settings on one line. Check Point Gaia returns effective user `password-hash`, SNMP community and SNMPv3 USM user lines after `delete` commands are applied. Check Point FW1 policy exports carry no device credentials and are rejected. On Windows, sensitive reports are created empty, restricted to the current user with `icacls`, and only then written; if that fails, the file is removed and no report is produced. Unsupported families are rejected; no generic raw-config dump or heuristic evidence substitution is performed. Sensitive reports are created only at a new path with exclusive creation and restrictive creation mode where the OS supports it. The user must still control directory permissions and distribution, especially on Windows where inherited ACLs govern access.
...
F5 BIG-IP TMOS currently has a basic saved tmsh/SCF text adapter. It tokenizes balanced object blocks, retains only selected sanitized management and audit properties, and applies last-explicit-property precedence. `sys sshd allow none` means unrestricted sources, while `sys httpd allow none` means no permitted clients; these native meanings stay separate. An explicit `auth password-policy max-login-failures 0` establishes disabled local lockout; `minimum-length 0` is assessed only with explicitly enabled policy enforcement. Explicit `auth user password` is distinct from `encrypted-password`: the former produces a secret-free plaintext-storage finding, while masked values remain unknown and the latter is not assumed strong. An explicit active remote `auth source` is compared with same-type provider objects; only when all exported providers explicitly set `servers none` is incomplete remote authentication reported. Active LDAP/certificate-LDAP with configured servers also reports explicit `ssl disabled` only when every exported same-type provider has that state; `ssl enabled` or `start-tls` alternatives suppress that finding. Those protected-transport modes receive a separate finding only when every such provider explicitly disables `ssl-check-peer`; absent or enabled peer checks do not produce that finding. Missing providers, external protected paths and local fallback policy remain unknown; user role and broader AAA semantics remain unmodeled. Omitted values and organization-specific finite thresholds remain ungraded. A narrow LTM adapter follows an explicitly enabled virtual server's partition-scoped clientside Client SSL reference to an enabled exported profile before assessing `allow-non-ssl enabled`. Missing properties, unresolved/inactive attachments and unverified defaults remain unknown or ungraded. Malformed or unrelated input produces a controlled parse-error report. Module checks (SC-024) read `sys provision`, `sys db` values including the AFM global default action (`tm.fw.defaultaction`, accept when absent per F5 ID813165), `asm policy` active/blocking-mode state and `ltm policy` actions that enable an ASM policy, followed from an enabled virtual server's `policies` list. Broader LTM traffic, APM, AFM rule contents and ASM signature semantics are not inferred from this slice.

Check Point Gaia OS (SC-023) is a separate family, `CHECKPOINT_GAIA`, for the Clish `show configuration` output. The FW1 policy export has no operating-system settings, so OS posture needs its own input rather than a companion file. `CheckPointGaiaParser` tokenizes `set`/`add`/`delete` commands with shell quoting; the last `set` for a setting wins and `delete` removes communities, SNMPv3 users and local users. Evidence redacts password hashes, pass phrases and community names and replaces banner text. Only failed-login lockout uses a documented default (off); every other check needs an explicit value. The `# Language version` header is the Clish syntax version, not the Gaia release, so `get_version()` returns unknown.
...
The companion IOS/XE EIGRP adapter is limited to classic numeric-AS, default-VRF IPv4 processes. It binds effective `network` selectors to configured interface addresses, resolves per-AS interface MD5 mode and key-chain bindings, and excludes shut, unmatched or passive interfaces. Unlike RIP, passive EIGRP does not exchange hellos or updates and cannot form an adjacency. Missing mode is unauthenticated; enabled MD5 without an exported effective key is unresolved; unsupported mode syntax is unknown. A named-mode stage covers default-VRF `address-family ipv4 [unicast] autonomous-system N` under `router eigrp <name>`: `af-interface default` settings apply to every interface and a specific `af-interface` overrides them (Cisco EIGRP command reference); af-interface `shutdown`/`passive-interface` exclude the interface; `authentication mode hmac-sha-256` is effective, MD5 needs a populated key chain, and a specific `no authentication ...` against an inherited default is unknown. VRF, IPv6 and multicast address families, classic address-family VRFs, and operational neighbor state require separate adapters or evidence. Shared key-chain parsing retains only key presence and redacted evidence.
...
Software advisories (PT-010) live in `src/advisories/`. `versions.py` maps a family and exported release to a CPE product and version, and refuses train-only or unsupported strings with a stable reason code. `nvd.py` confirms the exact release in the NVD CPE dictionary, then queries the CVE API with `cpeName` and `isVulnerable`, so NVD applies its own version-range matching. Requests are paginated, rate limited, time out, and use an injectable HTTP function. `service.py` builds neutral `SoftwareAdvisory` records (CVSS, CISA KEV date from NVD's `cisaExploitAdd`, and a conditional flag when every applicability statement ANDs the OS with another platform or component), plus a status record: `not-requested`, `completed`, `unavailable` or `error`. The lookup runs only when the CLI creates an `AdvisoryRequest` (`--cve-lookup` or a saved bundle through `--cve-data`). It never changes findings, rule IDs or snapshots, and the report always states its limitations. A version match is not configuration-backed exploitability. `lifecycle.py` (SC-022) maps a family and release to an endoflife.date product and release cycle (major.minor), fetches only the product document, and compares its end dates with the assessment date (`end-of-life`, `limited-support`, `supported`, `unlisted`). The document is saved in the bundle, so replays are reproducible. The project maintains no lifecycle list itself.
...
`src/report/report.py` renders findings and optional software advisories as HTML or JSON. The neutral `Finding.to_dict()` contract remains the issue JSON boundary. Plugins pass parser `ConfigEvidence` objects as finding evidence where available; `Finding` keeps the text in `evidence` and adds `evidence_locations` with text, line, file name and `line_origin`. Before filtering, every processor calls `attach_source_lines`, which fills a missing line only when the evidence text equals exactly one input line (`source-match`); repeated, redacted, summarized and absence statements keep no line. Reports never guess a location. Findings may also declare a `basis` (PT-009): explicit value, documented default, or missing explicit setting. The rule sets it where the finding is created; `to_dict()` adds it as `basis` (null when undeclared), and the report shows a short note on the card and adds `basis-note` in JSON. For a missing explicit setting, the note says the effective value may already be safe on some releases.

Reader-oriented explanations are presentation data, not detection logic. `src/analyze/common/guidance.py` maps each rule ID, by an ordered explicit list of patterns on the part after `<vendor>.<os>.`, to vendor-neutral layered text (plain summary, abuse example, technical background, what to verify) and to an *area*. Areas declare related areas; `src/report/explanations.py` orders findings by severity and lists, for each finding, the other findings in its related areas, or states that none was reported without implying the control is present. HTML shows these next to the finding's own observation, impact and recommendation; JSON adds `guidance` and `related-areas` to each `security-audit` entry. A test requires every rule ID in the analyzers to have guidance. `src/report/coverage.py` separately derives an additive, versioned ledger from `NormalizedConfig`: parser/input provenance, every normalized field's knowledge state and known item count, exclusion selection, and sanitized parser diagnostics. It never serializes normalized evidence.
